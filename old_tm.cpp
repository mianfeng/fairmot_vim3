...
static void nms_sorted_bboxes(const std::vector<Object>& faceobjects, std::vector<Object>& pick_objects, std::vector<int>& picked, float nms_threshold)
{
    picked.clear();
...
void get_input_data_uint8(cv::Mat& sample, uint8_t* input_data, int img_h, int img_w, const float* mean, const float* scale,
...
void VisualizeTrackResult(const cv::Mat& img,
                          const MOTResult& results, int time)
{
    ...
    for (int i = 0; i < results.size(); i++)
    {
        ...
        cv::Point id_pt = cv::Point(results[i].rects.left, results[i].rects.top + 20);
    ...
    static int NUM = 0;
    // NUM = time/100;
    static cv::VideoWriter video_out(video_path, 828601953, 20, cv::Size(vis_img.cols, vis_img.rows), true);
#ifdef CUT
    if (!(time % 400) || time == 1)
    {
        std::cout << video_path << "\n";
        video_path = video_name + "(" + std::__cxx11::to_string(NUM++) + ")" + ".mp4";
        cv::VideoWriter video_out1(video_path, 828601953, 30, cv::Size(vis_img.cols, vis_img.rows), true);
...
static void draw_objects_video(const cv::Mat& bgr, const std::vector<Object>& objects, int fourcc, double fps, int frame_width, int frame_height, int time)
{
    ...
    for (size_t i = 0; i < objects.size(); i = i + 1)
    {
        ...
        //         obj.rect.y, obj.rect.area(), obj.rect.y + obj.rect.height, class_names[obj.label]);
    ...
    static int NUM = 0;
    NUM = time / 100;
    static cv::VideoWriter video_out(video_path, fourcc, 16, cv::Size(image.cols, image.rows), true);
    if (!(time % 150) || time == 1)
    {
        std::cout << video_path << "\n";
        video_path = video_name + "(" + std::__cxx11::to_string(NUM) + ")" + ".mp4";
        cv::VideoWriter video_out1(video_path, fourcc, 16, cv::Size(image.cols, image.rows), true);
        video_out = video_out1;
        video_out1.release();
    }
...
void show_usage()
{
    fprintf(
        stderr,
        "[Usage]:  [-h]\n    [-m model_file] [-i image_file]  [-n ues webcam]\n");
    fprintf(
        stderr,
...
int video_input(const char* video_file, const char* model_file, float prob_threshold, int state)
{
    ...
    while (true)
    {
        ...
        get_input_data_uint8(frame, input_data.data(), img_h, img_w, mean, scale, input_scale, input_zero_point);
        ...
        heatmap_count = pool_count = get_tensor_buffer_size(pool_output) / sizeof(uint8_t);
        ...
        {
            // float heatmap_data = ((float)heatmap_data_u8[i] - (float)heatmap_zero_point) * heatmap_scale;
        ...
        for (auto& obj : objects)
        {
            ...
            float offset_x = ((float)offset_data_u8[obj.idx * 2 + 0] - (float)offset_zero_point) * offset_scale;
            float offset_y = ((float)offset_data_u8[obj.idx * 2 + 1] - (float)offset_zero_point) * offset_scale;
            xs += offset_x;
            ys += offset_y;

            float size_0 = ((float)size_data_u8[obj.idx * 4 + 0] - (float)size_zero_point) * size_scale;
            float size_1 = ((float)size_data_u8[obj.idx * 4 + 1] - (float)size_zero_point) * size_scale;
            float size_2 = ((float)size_data_u8[obj.idx * 4 + 2] - (float)size_zero_point) * size_scale;
            float size_3 = ((float)size_data_u8[obj.idx * 4 + 3] - (float)size_zero_point) * size_scale;
            float x1 = xs - size_0;
            float y1 = ys - size_1;
            float x2 = xs + size_2;
            float y2 = ys + size_3;
            ...
            {
                // obj.feat(0, i) = ((float)feat_data_u8[obj.idx * 128 + i] - (float)feat_zero_point) * feat_scale;
                obj.feat.at<float>(0, i) = ((float)feat_data_u8[obj.idx * 128 + i] - (float)feat_zero_point) * feat_scale;
            }
        ...
        // fprintf(stderr, "[L] %d\n",__LINE__);
        // result.clear();
        // draw_objects_video(frame, objects_pick,828601953,fps,frame_width,frame_height,time);
...
int main(int argc, char* argv[])
{
    ...
    if (nullptr != video_file)
    {
        if (video_file == "net")
        {
            ...
            {
            case 0:
                video_file = "rtsp://admin:abc123456@192.168.0.80:554/cam/realmonitor?channel=1&subtype=0";
                break;
            case 1:
                video_file = "rtsp://admin:abc123456@192.168.0.81:554/cam/realmonitor?channel=1&subtype=0";
                break;
            case 2:
                video_file = "rtsp://admin:abc123456@192.168.0.82:554/cam/realmonitor?channel=1&subtype=0";
                break;
            case 3:
