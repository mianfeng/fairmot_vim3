...
#define rect2vec4f(m) cv::Vec4f(m.x, m.y, m.x + m.width, m.y + m.height)
...
bool JDETracker::update(const std::vector<Object> &objects,
                        std::vector<Track> *tracks) {
    ++timestamp;
    TrajectoryPool candidates(objects.size());
    for (int i = 0; i < objects.size(); ++i) {
        float score = objects[i].prob;
        cv::Vec4f ltrb = rect2vec4f(objects[i].rect);
        const cv::Mat &embedding = objects[i].feat;
    ...
    for (miter = matches.begin(); miter != matches.end(); miter++) {
        ...
        activated_trajectories.push_back(
            unconfirmed_trajectories[miter->first]);
    }
    ...
    for (size_t i = 0; i < mismatch_row.size(); ++i) {
        ...
        removed_trajectories.push_back(
            unconfirmed_trajectories[mismatch_row[i]]);
    }
