...
cv::Mat VisualizeTrackResult(const cv::Mat& img, const MOTResult& results,
    ...
    // video_out(video_path,828601953,30,cv::Size(vis_img.cols,
    // vis_img.rows),true); #ifdef CUT if(!(time % 400)||time ==1)
    // {
    //     std::cout<<video_path<<"\n";
    //     video_path =  video_name + "(" +std::__cxx11::to_string(NUM++)+")"
    //     +".mp4";
    //  cv::VideoWriter
    //  video_out1(video_path,828601953,30,cv::Size(vis_img.cols,
    //  vis_img.rows),true);
...
int outFairmotGraph(FairmotGraph* fairmotgraph, cv::Mat& frame,
                    int frame_height, int frame_width, int time, void* out_ptr,
                    int size) {
    const float mean[3] = {0, 0, 0};
    const float scale[3] = {0.003921, 0.003921, 0.003921};

    cv::Mat yuvNV21;
    yuvNV21.create(frame_height * 3 / 2, frame_width, CV_8UC1);
    memcpy(yuvNV21.data, out_ptr, size);
    cvtColor(yuvNV21, frame, cv::COLOR_YUV2RGB_NV21);

    if (frame.empty() || time == 800) {
        fprintf(stderr, "run img empty\n");
        return -1;
    }
    /* prepare process input data, set the data mem to input tensor */
    get_input_data_uint8(frame, fairmotgraph->input_data.data(), img_h, img_w,
                         mean, scale, fairmotgraph->input_scale,
    ...
    /* get output tensor */
    double start_out = get_current_time();
    ...
    while (getFfmpegDemuxer(&demuxer) == 0) {
        ...
        do {
            ...
            if (decoder.size > 0) {
                ...
                if (outFairmotGraph(&fairmotgraph, frame, frame_height,
                                    frame_width, time, decoder.out_ptr,
                                    decoder.size) < 0) {
                    fprintf(stderr, "run graph failed \n");
                    goto close;
                    return -1;
                ...
                vis_img =
                    VisualizeTrackResult(frame, fairmotgraph.result, time);
                fairmotgraph.result.clear();
                double encode_start = get_current_time();
                if (!vis_img.empty()) {
                    if ((getVim3Encoder(&encoder, vis_img.data, frame.rows,
                                        frame.cols)) < 0)
